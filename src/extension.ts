...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerTextEditorCommand("syncScroll.jumpToNextPanelCorrespondingPosition", (textEditor) => {
      ...
      const textEditors = vscode.window.visibleTextEditors.filter((editor) => editor !== textEditor && editor.document.uri.scheme !== "output");
    ...
    vscode.commands.registerTextEditorCommand("syncScroll.copyToAllCorrespondingPositions", (textEditor) => {
      vscode.window.visibleTextEditors
        ...
        .forEach((scrolledEditor) => {
          scrolledEditor.edit((editBuilder) => textEditor.selections.map((selection) => editBuilder.replace(calculateRange(selection, offsetByEditors.get(scrolledEditor)), textEditor.document.getText(selection.isEmpty ? wholeLine(selection) : selection) + "\n")));
        });
    }),
      ...
      scrollingTask = setTimeout(() => {
        vscode.window.visibleTextEditors
          ...
          .forEach((scrolledEditor) => {
            ...
            scrolledEditor.revealRange(calculateRange(visibleRanges[0], offsetByEditors.get(scrolledEditor), textEditor, scrolledEditor), vscode.TextEditorRevealType.AtTop);
          });
      }, 0);
    }),
